...
def tv_loss(img, tv_weight):
    ...
    return tv_weight * ((tf.norm(h_shift_no_end - h_shift_no_start) ** 2)
                        + (tf.norm(w_shift_no_end - w_shift_no_start) ** 2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer in enumerate(style_layers):
        ...
        loss += style_weights[i] * (tf.norm(img_gram - style_targets[i]) ** 2)
...
def gram_matrix(features, normalize=True):
    ...
    reshape_feat = tf.reshape(features, [-1, C])  # [H * W, C]
    gram = tf.matmul(tf.transpose(reshape_feat), reshape_feat)
